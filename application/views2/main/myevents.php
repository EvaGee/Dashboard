...
<div class="panel panel-flat">
	...
	<table class="table datatable-basic">
		...
		<?php 
			...
			<tr>
				<td><a data-toggle="modal" data-target="#modal<?php echo($value['id']); ?>"><?php echo($value['event_title']); ?></a></td>
				<td><?php echo($value['event_venue']); ?></td>
				<td><?php echo($value['event_date']); ?></td>
				<td><?php echo($value['event_created_by']); ?></td>
				<td><?php echo($value['ipay_commission']); ?></td>
						...
						<ul class="icons-list">
			        		<li class="dropdown">
								...
								<ul class="dropdown-menu dropdown-menu-right">
									<li><a data-toggle="modal" data-target="#modal<?php echo($value['id']); ?>"> View More</a>
									</li>
									<li><a href="<?php echo base_url(); ?>index.php/my-ticket?sold=<?php echo($value['id']); ?>"> View Sold Tickets</a>
									</li>
									<?php
									if ($user_role == 1 && $value['event_status'] != 1) {
										?>
										<li><a href="<?php echo base_url(); ?>index.php/Welcome/approveEvent/<?php echo $value['id']; ?>"> Approve Event</a>
										</li>
										<?php
									}	if($value['account_id'] == $user_id && $value['event_status'] != 1 || $value['account_id'] == $user_id && $value['event_status'] == 1 && $user_role == 1) {
										?>
										<li><a href="<?php echo base_url(); ?>index.php?editid=<?php echo($value['id']); ?>"> Edit Ticket</a>
										</li>
										<?php
									}
									?>
